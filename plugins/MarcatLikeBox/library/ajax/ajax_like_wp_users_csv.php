...
if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
    ...
    $surch_obj->like_wp_users_search_csv();   
}
...
class ajax_like_wp_users_search {
    ...
    public function like_wp_users_search_csv() {
        global $wpdb;
        //全検索
        ...
        //CSV書き出し
        $output[0]['ID'] = 'ユーザーID';
        $output[0]['user_id'] = 'ユーザー名';
        $output[0]['user_email'] = 'メールアドレス';
        $output[0]['user_registered'] = '登録日';
        
        //$shinoutput = array();
        foreach($output as $key=>$val) {
            $shinoutput[]['ID'] = $like->ID;
            $shinoutput[]['user_id'] = $like->user_login;
            $shinoutput[]['user_email'] = $like->user_email;
            $shinoutput[]['user_registered'] = $like->user_registered;
        }
