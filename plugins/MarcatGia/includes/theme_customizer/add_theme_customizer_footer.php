...
function theme_footer_customizer( $wp_customize ) {
    ...
    $wp_customize->add_setting( footer_go_top_button );
    $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, footer_go_top_button, array(
        'label' => 'トップに戻るボタン', //設定ラベル
        'section' => footer_section, //セクションID
        'settings' => footer_go_top_button, //セッティングID
        'description' => 'トップに戻るボタンをアップロードして変えましょう。通常時を「ファイル名_off.png」ホバー時を「ファイル名_on.png」とすることで変更できます。',
    ) ) );
    
    
    $wp_customize->add_setting( footer_logo_pc_image );
    $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, footer_logo_pc_image, array(
        'label' => 'フッター用のロゴ画像（PC）', //設定ラベル
        'section' => footer_section, //セクションID
        'settings' => footer_logo_pc_image, //セッティングID
        'description' => 'フッター用のロゴ画像（PC）をアップロードして変えましょう。',
    ) ) );
    ...
    $wp_customize->add_control( new WP_Customize_Control( $wp_customize, svg_text_footer_box_01, array(
        'label'	=> 'フッター用のロゴ画像(SVGコード)',
        'section'	=> footer_section,
        'settings'	=> svg_text_footer_box_01,
        'type'          => 'textarea',
        'description' => 'フッター用のロゴ画像SVGコードで記載する場合はこちらに記載してください。こちらに入力した場合こちらが優先的に出力されます。',
    ) ) );
